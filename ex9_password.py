...
def check_auth_cookie(cookie):
    response = requests.get("https://playground.learnqa.ru/ajax/api/check_auth_cookie", cookies={"auth_cookie": cookie})
    return response.text

for password in unic_password:
    cookie = get_secret_password_homework(login, password)
    response_text = check_auth_cookie(cookie)
    if response_text != "You are NOT authorized":
        print(password, response_text)
